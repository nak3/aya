...
pub struct LwtInContext {
    pub skb: SkBuff,
}

impl LwtInContext {
    pub fn new(skb: *mut __sk_buff) -> LwtInContext {
        let skb = SkBuff { skb };
        LwtInContext { skb }
    }
    ...
    /// use core::mem;
    ///
    /// use aya_bpf::{bindings::TC_ACT_PIPE, macros::map, maps::PerCpuArray, programs::LwtInContext};
    ...
    ///
    /// fn try_classifier(ctx: LwtInContext) -> Result<i32, i32> {
    ///     let buf = unsafe {
...
impl BpfContext for LwtInContext {
